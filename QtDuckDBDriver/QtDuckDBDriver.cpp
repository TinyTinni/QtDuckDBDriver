...
void QDuckDBResultPrivate::initColumns(bool emptyResultset)
{
    ...
    for (int i = 0; i < nCols; ++i) {
        QString colName = QString::fromUtf8(
            sqlite3_column_name(stmt, i)
        ).remove(u'"');
        const QString tableName = QString::fromUtf8(
...
bool QDuckDBResult::prepare(const QString& query)
{
    ...
    int res = sqlite3_prepare_v2(d->drv_d_func()->access, query_string.constData(), query_string.size(),
        &d->stmt, (const char**)&pzTail);
...
QDuckDBDriver::QDuckDBDriver(sqlite3* connection, QObject* parent)
    : QSqlDriver(*new QDuckDBDriverPrivate, parent)
{
    Q_D(QDuckDBDriver);
    d->access = connection;
