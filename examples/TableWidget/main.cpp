...
int main(int argc, char *argv[]) {
	...
	auto start = std::chrono::high_resolution_clock::now();
	loading_stmt.exec();
	// db.exec("CREATE TABLE new_tbl AS SELECT * FROM
	// read_csv_auto('C:/Users/Anwender/source/repos/QtDuckDBDriver/QtDuckDBDriver/duckdb/data/csv/customer.csv');");
	...
	// fut.then(QtFuture::Launch::Sync, [view, model2, db]() {view->setModel(model2); view->update(); db.exec("DROP
	// TABLE new_tbl"); });
